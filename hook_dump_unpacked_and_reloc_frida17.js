...
function installSub167CHooks(m) {
  ...
  attachProbeOnce(mid, 'sub_167C after RELA/JMPREL before wipe', {
    onEnter() {
      const st = activeSub167cByTid[this.threadId];
      if (st === undefined) {
        log('[sub_167C mid] missing active state');
        return;
      }
      log(`[sub_167C mid] after RELA/JMPREL before symwipe, before RELR; load_bias=${st.loadBias} dynamic=${st.dynamic}`);
      dumpRelocCombinedImage('after_rela_jmprel_before_symwipe_no_relr', st.loadBias);
    }
  });
}
