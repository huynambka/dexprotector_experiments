...
const TRACE_SUB5F0D0_INTERNALS = false;
const DUMP_CLASSES_DEX_CONTAINER = true; // dump container immediately after sub_3EA28, before sub_3EB6C munmap() hides pages
const DUMP_CLASSES_DEX_CHUNKS = true; // dump valid DEX chunks produced from assets/classes.dex.dat
...
const ENABLE_NATIVE_EXCEPTION_HANDLER = false; // ART uses SIGSEGV for implicit Java null checks; this is too noisy and can disturb handling.
const ENABLE_JAVA_CRASH_LOGGER = true;
...
function readZipEntryName(entry) {
  try {
    if (!entry || entry.isNull()) return '<null-entry>';
    const nameLen = entry.add(0x1c).readU16();
    return readAsciiLen(entry.add(0x2e), nameLen);
  } catch (_) { return '<bad-entry>'; }
}

function readZipEntryCrc(entry) {
  try { return entry.add(0x10).readU32() >>> 0; } catch (_) { return null; }
}

function readZipEntrySize(entry) {
  try { return entry.add(0x18).readU32() >>> 0; } catch (_) { return null; }
}
...
function installCrashTraceHooks() {
  if (crashTraceInstalled) return;
  crashTraceInstalled = true;

  if (ENABLE_NATIVE_EXCEPTION_HANDLER) {
    try {
      Process.setExceptionHandler(function (d) {
        const ctx = d.context || {};
        const pc = ctx.pc || ptr(0);
        const lr = ctx.lr || ptr(0);
        const mem = d.memory ? ` mem=${d.memory.operation}@${d.memory.address}` : '';
        log(`\n[PROCESS EXCEPTION] type=${d.type} address=${d.address}${mem} pc=${moduleDesc(pc)} lr=${moduleDesc(lr)}`);
        log(`[PROCESS EXCEPTION bt] ${shortBacktrace(ctx)}`);
        return false; // keep normal crash behavior; only log.
      });
      log('[hooked] Process exception logger');
    } catch (e) {
      warn(`[exception logger FAIL] ${e}`);
    }
  } else {
    log('[skip] native Process exception logger disabled; avoids ART implicit-null-check noise');
  }
  ...
  [
    ...
    if (addr === null) return;
    attachOnce(addr, `process death ${name}`, {
      onEnter(args) {
        const vals = [];
        for (let i = 0; i < argc; i++) vals.push(String(args[i]));
        log(`\n[PROCESS DEATH] ${name}(${vals.join(', ')}) caller=${moduleDesc(this.context.lr)}`);
        log(`[PROCESS DEATH bt] ${shortBacktrace(this.context)}`);
      }
    });
  });
}

function installJavaCrashLogger() {
  if (!ENABLE_JAVA_CRASH_LOGGER) return;
  if (typeof Java === 'undefined' || !Java.available) {
    warn('[java crash logger] Java not available yet');
    return;
  }
  ...
  Java.perform(function () {
    ...
    function throwableToString(e) {
      try { return String(e); } catch (_) { return '<bad throwable>'; }
    }

    function stack(e) {
      try { return Log.getStackTraceString(e); } catch (ex) { return `<stack failed: ${ex}>`; }
    }

    try {
      const KH = Java.use('com.android.internal.os.RuntimeInit$KillApplicationHandler');
      const uncaught = KH.uncaughtException.overload('java.lang.Thread', 'java.lang.Throwable');
      uncaught.implementation = function (t, e) {
        let tname = '<thread>';
        try { tname = t.getName(); } catch (_) {}
        log(`\n[JAVA UNCAUGHT KillApplicationHandler] thread=${tname} throwable=${throwableToString(e)}`);
        log(stack(e));
        return uncaught.call(this, t, e);
      };
      log('[hooked] Java RuntimeInit$KillApplicationHandler.uncaughtException');
    } catch (e) {
      warn(`[java crash logger] KillApplicationHandler hook failed: ${e}`);
    }

    try {
      const Thread = Java.use('java.lang.Thread');
      const dispatch = Thread.dispatchUncaughtException.overload('java.lang.Throwable');
      dispatch.implementation = function (e) {
        let tname = '<thread>';
        try { tname = this.getName(); } catch (_) {}
        log(`\n[JAVA UNCAUGHT Thread.dispatch] thread=${tname} throwable=${throwableToString(e)}`);
        log(stack(e));
        return dispatch.call(this, e);
      };
      log('[hooked] Java Thread.dispatchUncaughtException');
    } catch (e) {
      warn(`[java crash logger] Thread.dispatch hook failed: ${e}`);
    }
  });
}
...
function installCrcTraceHooks(loadBias) {
  // Our Interceptor on hidden sub_4E354 entry may perturb LR, and sub_4E354 saves LR into qword_8CEF0.
  // sub_5F0D0 later calls sub_4E348()->qword_8CEF0 and uses /proc/self/maps to recover the outer
  // libdexprotector.so path. If qword_8CEF0 points to a Frida/anonymous trampoline, that path becomes
  // empty and sub_5F0D0 returns 731 before CRC checks. Restore the original outer return address.
  attachOnce(loadBias.add(CS_AFTER_STORE_QWORD_8CEF0), 'restore qword_8CEF0 outer return address', {
    onEnter() {
      ...
      let before = null;
      try { before = slot.readPointer(); slot.writePointer(outerHiddenReturnAddress); } catch (e) { warn(`[qword_8CEF0 fix] failed: ${e}`); return; }
      log(`[qword_8CEF0 fix] before=${before} forced=${outerHiddenReturnAddress}`);
    }
  });

  if (!TRACE_SUB5F0D0_INTERNALS) {
    log('[trace] sub_5F0D0 internals disabled; only probing sub_4EB9C onEnter');
    return;
  }

  const sub5f0d0 = loadBias.add(H_SUB_5F0D0_RVA);
  const sub5f46c = loadBias.add(H_SUB_5F46C_RVA);
  const pathState = {};

  attachOnce(sub5f0d0, 'sub_5F0D0 native-lib CRC watchlist', {
    onEnter(args) {
      this.id = ++seq;
      this.zipInfo = args[0];
      log(`\n[sub_5F0D0 enter #${this.id}] apk_zip_info=${this.zipInfo} caller=${moduleDesc(this.context.lr)}`);
    },
    onLeave(retval) {
      log(`[sub_5F0D0 leave #${this.id}] ret=${retval} (${retval.toInt32 ? retval.toInt32() : retval})`);
    }
  });

  // Path found by sub_3C0EC(qword_8CEF0, ..., &path): mapping path containing outer libdexprotector.so.
  attachOnce(loadBias.add(CS_5F0D0_MAP_PATH_STRLEN), 'sub_5F0D0 mapped path before strlen', {
    onEnter() {
      const path = safeCString(this.context.x0);
      pathState[this.threadId] = { ptr: this.context.x0, path };
      log(`[sub_5F0D0 path] ${JSON.stringify(path)}`);
    }
  });

  attachOnce(loadBias.add(CS_5F0D0_AFTER_PATH_STRLEN), 'sub_5F0D0 after strlen(path)', {
    onEnter() {
      const st = pathState[this.threadId] || {};
      const len = this.context.x0.toUInt32();
      let last4 = '';
      try { if (st.ptr && len >= 4) last4 = readAsciiLen(st.ptr.add(len - 4), 4); } catch (_) {}
      log(`[sub_5F0D0 strlen] len=${len} last4=${JSON.stringify(last4)} path=${JSON.stringify(st.path)}`);
    }
  });

  attachOnce(loadBias.add(CS_5F0D0_AFTER_APK_K_CMP), 'sub_5F0D0 after .apk final cmp', {
    onEnter() {
      log(`[sub_5F0D0 .apk check] last_char_w8=0x${(this.context.x8.toUInt32() & 0xff).toString(16)} expect=0x6b`);
    }
  });

  if (TRACE_TIGHT_SUB5F0D0_BRANCHES) {
    attachOnce(loadBias.add(CS_5F0D0_SUCCESS_RET0), 'sub_5F0D0 success ret0', {
      onEnter() {
        log(`[sub_5F0D0 SUCCESS] returning 0 tid=${this.threadId}`);
      }
    });

    attachOnce(loadBias.add(CS_5F0D0_SUCCESS_BRANCH), 'sub_5F0D0 success branch to epilogue', {
      onEnter() {
        log(`[sub_5F0D0 POST-SUCCESS] hit 0x5f360 tid=${this.threadId} x0=${this.context.x0} lr=${moduleDesc(this.context.lr)}`);
      }
    });

    attachOnce(loadBias.add(CS_5F0D0_EPILOGUE), 'sub_5F0D0 epilogue begin', {
      onEnter() {
        log(`[sub_5F0D0 EPILOGUE] tid=${this.threadId} x0=${this.context.x0} sp=${this.context.sp} lr=${moduleDesc(this.context.lr)}`);
      }
    });

    attachOnce(loadBias.add(CS_5F0D0_RET), 'sub_5F0D0 ret instruction', {
      onEnter() {
        log(`[sub_5F0D0 RET] tid=${this.threadId} x0=${this.context.x0} returning_to=${moduleDesc(this.context.lr)}`);
      }
    });
  }

  attachOnce(loadBias.add(CS_5F0D0_INIT_ZIP_ITER), 'sub_5F0D0 init ZIP iterator', {
    onEnter() {
      log(`[sub_5F0D0 zip-iter] calling sub_3C940 apk_zip_info=${this.context.x0} iter=${this.context.x1}`);
    }
  });

  // Common 731 return in sub_5F0D0. LR tells which path reached the error block less reliably than PC,
  // but logging PC hit helps see whether failure happens before native-lib CRC thread is built.
  attachOnce(loadBias.add(CS_5F0D0_ERROR_731), 'sub_5F0D0 hit error 731', {
    onEnter() {
      log(`[sub_5F0D0 ERROR] hit loc_5F1D8 -> return 731; lr=${moduleDesc(this.context.lr)}`);
    }
  });

  // sub_5F0D0 reads native lib CRC32 directly from APK ZIP central directory.
  attachOnce(loadBias.add(CS_5F0D0_NATIVE_CRC_READ), 'sub_5F0D0 native CRC from APK central-dir', {
    onEnter() {
      const ent = this.context.x21; // central directory entry pointer
      const name = readZipEntryName(ent);
      const crc = readZipEntryCrc(ent);
      const size = readZipEntrySize(ent);
      log(`[native central-dir CRC] name=${JSON.stringify(name)} crc=${fmtU32(crc)} size=0x${(size === null ? 0 : size).toString(16)} entry=${ent}`);
    }
  });

  // sub_5E684 stores CRC32 values for ic.dat-listed APK entries into crc_array[index].
  attachOnce(loadBias.add(CS_5E684_ICDAT_CRC_STORE), 'sub_5E684 ic.dat listed CRC store', {
    onEnter() {
      const idx = this.context.x8.toUInt32();
      const crc = this.context.x28.toUInt32();
      const crcArray = this.context.x21;
      const centralName = readAsciiLen(this.context.x23, this.context.x22.toUInt32());
      let icName = '<unknown>';
      try {
        const rec = this.context.x20;
        const p = rec.add(0x38).readPointer(); // name pointer stored in hash record
        icName = safeCString(p) || '<non-cstr>';
      } catch (_) {}
      log(`[ic.dat CRC store] index=${idx} crc=${fmtU32(crc)} central=${JSON.stringify(centralName)} ic_name=${JSON.stringify(icName)} dst=${crcArray.add(idx * 4)}`);
    }
  });

  // Final SipHash over collected CRC32 array compared with expected hash from ic.dat header.
  attachOnce(loadBias.add(CS_5E684_SIPHASH_COMPARE), 'sub_5E684 CRC-array SipHash compare', {
    onEnter() {
      log(`[ic.dat CRC SipHash] expected=${this.context.x8} computed=${this.context.x0} match=${ptrEq(this.context.x8, this.context.x0)}`);
    }
  });

  // sub_5F46C opens a real native-lib path, mmaps it, computes CRC32 via sub_771EC, compares with central-dir CRC.
  attachOnce(sub5f46c, 'sub_5F46C compute actual native CRC32', {
    onEnter(args) {
      this.id = ++seq;
      this.rec = args[0];
      let path = '<bad-record>';
      ...
      let size = null;
      try {
        path = safeCString(this.rec.readPointer()) || '<null-path>';
        expected = this.rec.add(8).readU32() >>> 0;
        size = this.rec.add(0x10).readU64();
      } catch (_) {}
      this.path = path;
      this.expected = expected;
      log(`\n[sub_5F46C enter #${this.id}] path=${JSON.stringify(path)} expected_crc=${fmtU32(expected)} expected_size=${size} caller=${moduleDesc(this.context.lr)}`);
    },
    onLeave(retval) {
      log(`[sub_5F46C leave #${this.id}] ret=${retval} (${retval.toInt32 ? retval.toInt32() : retval}) path=${JSON.stringify(this.path)}`);
    }
  });

  attachOnce(loadBias.add(CS_5F46C_COMPUTED_CRC), 'sub_5F46C computed CRC32 compare', {
    onEnter() {
      const rec = this.context.x19;
      let path = '<bad-record>';
      let expected = null;
      try {
        path = safeCString(rec.readPointer()) || '<null-path>';
        expected = rec.add(8).readU32() >>> 0;
      } catch (_) {}
      const computed = this.context.x9.toUInt32();
      log(`[native computed CRC] path=${JSON.stringify(path)} expected=${fmtU32(expected)} computed=${fmtU32(computed)} match=${expected !== null && ((expected >>> 0) === (computed >>> 0))}`);
    }
  });

  if (TRACE_AFTER_SUB5F0D0) {
  // Very tight trace after sub_5F0D0 success. Expected control flow:
  //   sub_5F0D0 ret -> 0x5EE6C -> 0x5E6C8 -> 0x5E730 cleanup -> sub_5E684 ret
  //   -> sub_4E354+0x4E7A8 -> sub_4EB9C
  attachOnce(loadBias.add(CS_5E684_AFTER_SUB5F0D0), 'POST sub_5F0D0 return in sub_5E684', {
    onEnter() {
      log(`[POST 5F0D0] returned x0=${this.context.x0} (${this.context.x0.toInt32()}); next=B loc_5E6C8 cleanup/return`);
    }
  });

  attachOnce(loadBias.add(CS_5E684_RETURN_VALUE_GATE), 'POST sub_5E684 common retval gate', {
    onEnter() {
      log(`[POST 5F0D0] at loc_5E6C8: x0=${this.context.x0} (${this.context.x0.toInt32()}) -> W20 return code`);
    }
  });

  attachOnce(loadBias.add(CS_5E684_CLEANUP_BEGIN), 'POST sub_5E684 cleanup begin', {
    onEnter() {
      let off8d238 = null, q8d230 = null, off8d240 = null, q8d248 = null;
      try { off8d238 = loadBias.add(ptr('0x8d238')).readPointer(); } catch (_) {}
      try { q8d230 = loadBias.add(ptr('0x8d230')).readPointer(); } catch (_) {}
      try { off8d240 = loadBias.add(ptr('0x8d240')).readPointer(); } catch (_) {}
      try { q8d248 = loadBias.add(ptr('0x8d248')).readPointer(); } catch (_) {}
      log(`[POST 5F0D0] sub_5E684 cleanup begin ret_code(W20)=${this.context.x20.toInt32()} off_8D238=${off8d238} qword_8D230=${q8d230} off_8D240=${off8d240} qword_8D248=${q8d248}`);
    }
  });

  attachOnce(loadBias.add(CS_5E684_BEFORE_RET), 'POST sub_5E684 before ret', {
    onEnter() {
      log(`[POST 5F0D0] sub_5E684 about to return W20=${this.context.x20} (${this.context.x20.toInt32()})`);
    }
  });

  attachOnce(loadBias.add(CS_4E354_AFTER_SUB5E684), 'POST sub_5E684 return in sub_4E354', {
    onEnter() {
      log(`[POST 5E684] back in sub_4E354: x0=${this.context.x0} (${this.context.x0.toInt32()}); next CBNZ decides error vs sub_4EB9C`);
    }
  });

  attachOnce(loadBias.add(CS_4E354_SUB5E684_OK_CONTINUE), 'POST sub_5E684 success continues', {
    onEnter() {
      log('[POST 5E684] returned 0; clearing byte_8CEE8 and preparing sub_4EB9C');
    }
  });

  attachOnce(loadBias.add(CS_4E354_BEFORE_SUB4EB9C), 'POST before sub_4EB9C from sub_4E354', {
    onEnter() {
      log(`[POST 5E684] calling sub_4EB9C env=${this.context.x0} v54=${this.context.x1} assetManager=${this.context.x2}`);
    }
  });
  }
}
...
function ret32(ctx) {
  try { return ctx.x0.toInt32(); } catch (_) { return 0; }
}

function retRaw(ctx) {
  try { return String(ctx.x0); } catch (_) { return '<bad-x0>'; }
}

function installFinalGateTraceHooks(loadBias) {
  const sub4eb9c = loadBias.add(H_SUB_4EB9C_RVA);
  const sub4efb0 = loadBias.add(H_SUB_4EFB0_RVA);

  function installJniProbe(env) {
    try {
      if (!env || env.isNull()) return;
      const table = env.readPointer();
      const findClass = table.add(JNI_FIND_CLASS_OFF).readPointer();
      const newGlobalRef = table.add(JNI_NEW_GLOBAL_REF_OFF).readPointer();
      const getMethodID = table.add(JNI_GET_METHOD_ID_OFF).readPointer();
      const getStaticFieldID = table.add(JNI_GET_STATIC_FIELD_ID_OFF).readPointer();
      const newStringUTF = table.add(JNI_NEW_STRING_UTF_OFF).readPointer();
      const newIntArray = table.add(JNI_NEW_INT_ARRAY_OFF).readPointer();
      const registerNatives = table.add(JNI_REGISTER_NATIVES_OFF).readPointer();
      const exceptionCheck = table.add(JNI_EXCEPTION_CHECK_OFF).readPointer();

      function fromSub65(ctx) {
        const c = moduleDesc(ctx.lr);
        return c.indexOf('hidden+0x65') === 0 || c.indexOf('hidden+0x66') === 0;
      }

      attachOnce(findClass, 'JNI FindClass probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          this.name = safeCString(args[1]);
          log(`[JNI FindClass enter] name=${JSON.stringify(this.name)} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI FindClass leave] name=${JSON.stringify(this.name)} class=${retval}`);
        }
      });

      attachOnce(newGlobalRef, 'JNI NewGlobalRef probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          log(`[JNI NewGlobalRef enter] obj=${args[1]} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI NewGlobalRef leave] global=${retval}`);
        }
      });

      attachOnce(registerNatives, 'JNI RegisterNatives probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          this.clazz = args[1];
          ...
          log(`[JNI RegisterNatives enter] clazz=${this.clazz} methods=${this.methods} count=${this.count} caller=${moduleDesc(this.context.lr)}`);
          for (let i = 0; i < this.count && i < 8; i++) {
            const rec = this.methods.add(i * Process.pointerSize * 3);
            let name = null, sig = null, fn = ptr(0);
            try { name = safeCString(rec.readPointer()); } catch (_) {}
            try { sig = safeCString(rec.add(Process.pointerSize).readPointer()); } catch (_) {}
            try { fn = rec.add(Process.pointerSize * 2).readPointer(); } catch (_) {}
            log(`  [native ${i}] name=${JSON.stringify(name)} sig=${JSON.stringify(sig)} fn=${moduleDesc(fn)}`);
          }
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI RegisterNatives leave] ret=${retval} signed=${retval.toInt32()}`);
        }
      });

      attachOnce(getMethodID, 'JNI GetMethodID probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          this.name = safeCString(args[2]);
          this.sig = safeCString(args[3]);
          log(`[JNI GetMethodID enter] class=${args[1]} name=${JSON.stringify(this.name)} sig=${JSON.stringify(this.sig)} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI GetMethodID leave] name=${JSON.stringify(this.name)} sig=${JSON.stringify(this.sig)} method=${retval}`);
        }
      });

      attachOnce(getStaticFieldID, 'JNI GetStaticFieldID probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          this.name = safeCString(args[2]);
          this.sig = safeCString(args[3]);
          log(`[JNI GetStaticFieldID enter] class=${args[1]} name=${JSON.stringify(this.name)} sig=${JSON.stringify(this.sig)} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI GetStaticFieldID leave] name=${JSON.stringify(this.name)} sig=${JSON.stringify(this.sig)} field=${retval}`);
        }
      });

      attachOnce(newStringUTF, 'JNI NewStringUTF probe for sub_65B50', {
        onEnter(args) {
          this.on = fromSub65(this.context);
          if (!this.on) return;
          this.s = safeCString(args[1]);
          log(`[JNI NewStringUTF enter] s=${JSON.stringify(this.s)} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.on) return;
          log(`[JNI NewStringUTF leave] ret=${retval} s=${JSON.stringify(this.s)}`);
        }
      });

      attachOnce(newIntArray, 'JNI NewIntArray probe for sub_4EFB0', {
        onEnter(args) {
          this.fromFinal = hiddenBase !== null && moduleDesc(this.context.lr).indexOf('hidden+0x4ef') === 0;
          if (!this.fromFinal) return;
          this.len = u64ToSafeNumber(args[1]);
          log(`[JNI NewIntArray enter] len=${this.len} caller=${moduleDesc(this.context.lr)}`);
        },
        onLeave(retval) {
          if (!this.fromFinal) return;
          log(`[JNI NewIntArray leave] ret=${retval}`);
        }
      });

      attachOnce(exceptionCheck, 'JNI ExceptionCheck probe for sub_4EFB0', {
        onEnter(args) {
          const caller = moduleDesc(this.context.lr);
          // Log every ExceptionCheck from sub_4EFB0 and its nearby helper range.
          this.fromFinal = caller.indexOf('hidden+0x4e') === 0 || caller.indexOf('hidden+0x4f') === 0 || caller.indexOf('hidden+0x5') === 0 || caller.indexOf('hidden+0x6') === 0;
          if (!this.fromFinal) return;
          this.caller = caller;
        },
        onLeave(retval) {
          if (!this.fromFinal) return;
          log(`[JNI ExceptionCheck leave] ret=${retval} signed=${retval.toInt32()} caller=${this.caller}`);
        }
      });
    } catch (e) {
      warn(`[JNI probe install failed] ${e}`);
    }
  }


  function sanitizeFilePart(s) {
    if (s === null || s === undefined || s.length === 0) return 'unknown';
    return String(s).replace(/^\/+/, '').replace(/[^A-Za-z0-9_.-]+/g, '_');
  }

  let dexDumpIndex = 0;
  const dumpedDexChunks = new Set();

  function dumpDexChunksFromClassesContainer(base, size, tag) {
    try {
      if (!base || base.isNull() || size < 0x100) return;
      const end = base.add(size);
      const chunkCount = end.add(-36).readU32() >>> 0;
      if (chunkCount === 0 || chunkCount > 128) {
        warn(`[classes.dex.dat container] suspicious chunk_count=${chunkCount} size=0x${size.toString(16)} tag=${tag}`);
        return;
      }
      const offTable = end.add(-36 - chunkCount * 4);
      let rangeCount = 0;
      try { rangeCount = offTable.add(-4).readU32() >>> 0; } catch (_) {}
      log(`[classes.dex.dat container] tag=${tag} base=${base} size=0x${size.toString(16)} chunk_count=${chunkCount} range_count=${rangeCount} off_table=${offTable}`);
      for (let i = 0; i < chunkCount; i++) {
        const off = offTable.add(i * 4).readU32() >>> 0;
        if (off >= size) {
          log(`  [classes chunk ${i}] off=0x${off.toString(16)} out-of-container`);
          continue;
        }
        const dexPtr = base.add(off);
        const magic = hexBytes(readBytes(dexPtr, 8));
        let fileSize = 0;
        try { fileSize = dexPtr.add(0x20).readU32() >>> 0; } catch (_) { fileSize = 0; }
        const okMagic = magic !== null && magic.indexOf('6465780a') === 0;
        const okSize = fileSize >= 0x70 && fileSize <= 0x08000000;
        const inBounds = okSize && (off + fileSize <= size);
        log(`  [classes chunk ${i}] off=0x${off.toString(16)} ptr=${dexPtr} magic=${magic} file_size=0x${fileSize.toString(16)} in_bounds=${inBounds}`);
        if (!okMagic || !okSize || !inBounds) continue;
        const key = `${dexPtr}:${fileSize}`;
        if (dumpedDexChunks.has(key)) continue;
        dumpedDexChunks.add(key);
        const name = `classes_dex_dat_${String(dexDumpIndex++).padStart(2, '0')}_classes${i}_from_container.dex`;
        try { dumpMemoryToFile(name, dexPtr, fileSize); }
        catch (e) { warn(`[classes chunk ${i}] dump failed: ${e}`); }
      }
    } catch (e) {
      warn(`[classes.dex.dat container] parse failed: ${e}`);
    }
  }

  function installClassesDatContainerDumper() {
    if (!DUMP_CLASSES_DEX_CONTAINER) return;
    const sub3ea28 = loadBias.add(H_SUB_3EA28_RVA);
    attachOnce(sub3ea28, 'sub_3EA28 dump decrypted classes.dex.dat container', {
      onEnter(args) {
        this.enabled = moduleDesc(this.context.lr).indexOf('hidden+0x3ec') === 0; // called from sub_3EB6C
        if (!this.enabled) return;
        this.outPtrPtr = args[3];
        this.outSizePtr = args[4];
        log(`[classes.dex.dat container decrypt enter] caller=${moduleDesc(this.context.lr)} in=${args[1]} len=${args[2]} outPtrPtr=${this.outPtrPtr} outSizePtr=${this.outSizePtr}`);
      },
      onLeave(retval) {
        if (!this.enabled) return;
        const ret = retval.toInt32();
        log(`[classes.dex.dat container decrypt leave] ret=${ret}`);
        if (ret !== 0) return;
        try {
          const out = this.outPtrPtr.readPointer();
          let size = u64ToSafeNumber(this.outSizePtr.readPointer());
          if (size <= 0 || size > 0x10000000) {
            try { size = this.outSizePtr.readU32() >>> 0; } catch (_) {}
          }
          log(`[classes.dex.dat container decrypted] out=${out} size=0x${size.toString(16)}`);
          if (size > 0 && size <= 0x10000000) {
            try { dumpMemoryToFile('classes_dex_dat_decrypted_container.bin', out, size); }
            catch (e) { warn(`[classes.dex.dat container] full dump failed: ${e}`); }
            dumpDexChunksFromClassesContainer(out, size, 'sub_3EA28');
          }
        } catch (e) {
          warn(`[classes.dex.dat container] onLeave failed: ${e}`);
        }
      }
    });
  }

  function installClassesDexChunkDumper() {
    if (!DUMP_CLASSES_DEX_CHUNKS) return;
    const sub3f158 = loadBias.add(H_SUB_3F158_RVA);
    attachOnce(sub3f158, 'sub_3F158 dump DEX chunks from classes.dex.dat', {
      onEnter(args) {
        this.enabled = false;
        const caller = moduleDesc(this.context.lr);
        // sub_3EB6C calls sub_3F158 at hidden+0x3ee54 when processing assets/classes.dex.dat.
        if (caller.indexOf('hidden+0x3ee') !== 0 && caller.indexOf('hidden+0x3ed') !== 0) return;
        const dexPtr = args[1];
        const loc = safeCString(args[2]); // usually "/classes0", "/classes1", ...
        let magic = null;
        let size = 0;
        try { magic = hexBytes(readBytes(dexPtr, 8)); } catch (_) {}
        try { size = dexPtr.add(0x20).readU32() >>> 0; } catch (_) { size = 0; }
        const okMagic = magic !== null && magic.indexOf('6465780a') === 0; // "dex\n"
        const okSize = size >= 0x70 && size <= 0x08000000;
        log(`[classes.dex.dat DEX] caller=${caller} loc=${JSON.stringify(loc)} ptr=${dexPtr} magic=${magic} file_size=0x${size.toString(16)} ok=${okMagic && okSize}`);
        if (!okMagic || !okSize) return;
        const key = `${dexPtr}:${size}`;
        if (dumpedDexChunks.has(key)) return;
        dumpedDexChunks.add(key);
        const name = `classes_dex_dat_${String(dexDumpIndex++).padStart(2, '0')}_${sanitizeFilePart(loc)}.dex`;
        try { dumpMemoryToFile(name, dexPtr, size); }
        catch (e) { warn(`[classes.dex.dat DEX] dump failed: ${e}`); }
      }
    });
  }

  installClassesDatContainerDumper();
  installClassesDexChunkDumper();

  function traceChild(rvaHex, name) {
    const addr = loadBias.add(ptr(rvaHex));
    attachOnce(addr, `sub_4EFB0 child ${name}`, {
      onEnter(args) {
        ...
        if (!this.enabled) return;
        this.id = ++seq;
        log(`[child enter #${this.id}] ${name} caller=${this.caller} x0=${args[0]} x1=${args[1]} x2=${args[2]} x3=${args[3]}`);
      },
      onLeave(retval) {
        if (!this.enabled) return;
        let signed = '<err>';
        try { signed = retval.toInt32(); } catch (_) {}
        log(`[child leave #${this.id}] ${name} ret=${retval} signed=${signed}${signed !== 0 ? ' FAIL' : ' OK'}`);
      }
    });
  }

  // Function-level probes only. No mid-basic-block hooks.
  // Goal: find which sub_4EFB0 module returns 0x2e/46.
  [
    ['0x4f44c', 'sub_4F44C hidden Java payload loader'],
    ['0x3eb6c', 'sub_3EB6C classes/chunk loader'],
    ['0x402e0', 'sub_402E0 asset4/payload loader'],
    ['0x55718', 'sub_55718 string/native s(String) setup'],
    ['0x65b50', 'sub_65B50 QrGen$Segment setup'],
    ['0x66480', 'sub_66480 optional module'],
    ['0x51e4c', 'sub_51E4C AssetManager/openNonAsset bridge'],
    ['0x53b24', 'sub_53B24 optional module'],
    ['0x53390', 'sub_53390 asset2/runtime blob'],
    ['0x4ff68', 'sub_4FF68 optional module'],
    ['0x56578', 'sub_56578 optional module'],
    ['0x3a6a0', 'sub_3A6A0 optional module'],
    ['0x3a404', 'sub_3A404 optional module'],
    ['0x51a10', 'sub_51A10 AssetExtractor bridge'],
    ['0x56134', 'sub_56134 optional module'],
    ['0x3e188', 'sub_3E188 optional module'],
    ['0x35be8', 'sub_35BE8 R$string callback'],
  ].forEach(([rva, name]) => traceChild(rva, name));

  // Minimal: function-level only, no post-5F0D0/callsite hooks.
  attachOnce(sub4eb9c, 'sub_4EB9C ENTER/LEAVE ONLY', {
    onEnter(args) {
      this.id = ++seq;
      log(`\n[HIT sub_4EB9C #${this.id}] env=${args[0]} v54=${args[1]} assetManager=${args[2]} caller=${moduleDesc(this.context.lr)}`);
    },
    onLeave(retval) {
      let signed = '<err>';
      try { signed = retval.toInt32(); } catch (_) {}
      log(`[LEAVE sub_4EB9C #${this.id}] ret=${retval} signed=${signed}${signed !== 0 ? ' FAIL' : ' OK'}`);
    }
  });

  attachOnce(sub4efb0, 'sub_4EFB0 ENTER/LEAVE ONLY', {
    onEnter(args) {
      this.id = ++seq;
      log(`\n[HIT sub_4EFB0 #${this.id}] env=${args[0]} v54=${args[1]} assetManager=${args[2]} caller=${moduleDesc(this.context.lr)}`);
      installJniProbe(args[0]);
    },
    onLeave(retval) {
      let signed = '<err>';
      try { signed = retval.toInt32(); } catch (_) {}
      log(`[LEAVE sub_4EFB0 #${this.id}] ret=${retval} signed=${signed}${signed !== 0 ? ' FAIL' : ' OK'}`);
    }
  });
}
...
function installHiddenHooks(loadBias) {
  ...
  installFinalGateTraceHooks(loadBias);
...
function installOuterHooks(path) {
  ...
  attachOnce(m.base.add(DP_JNI_ONLOAD_RVA), 'outer JNI_OnLoad entry/leave', {
    onEnter(args) {
      ...
      log(`\n[outer JNI_OnLoad enter #${this.id}] vm=${args[0]} reserved=${args[1]} dword_B228=${status} off_B230=${entry}`);
    },
    onLeave(retval) {
