...
function installSub918Hooks(m) {
  ...
  attachProbeOnce(subDAC, 'sub_DAC decrypted out', {
    onEnter(args) {
      ...
      this.input = args[2];
      this.output = args[3];
      this.lrOff = this.context.lr.sub(m.base);
      this.printLen = this.len;
      if (this.printLen < 0) this.printLen = 0;
      if (this.printLen > DAC_MAX_PRINT) this.printLen = DAC_MAX_PRINT;
    },
    onLeave(retval) {
      const bytes = readBytes(this.output, this.printLen);
      const full = this.len > DAC_MAX_PRINT ? `/${this.len.toString(16)}` : '';
      if (bytes === null) {
        log(`[sub_DAC out] caller=${TARGET_LIB}+0x${this.lrOff.toString(16)} ctx=${this.ctx} in=${describeAddress(this.input)} out=${this.output} len=0x${this.len.toString(16)} read_failed`);
        return;
      }

      log(`[sub_DAC out] caller=${TARGET_LIB}+0x${this.lrOff.toString(16)} ctx=${this.ctx} in=${describeAddress(this.input)} out=${this.output} len=0x${this.len.toString(16)} show=0x${this.printLen.toString(16)}${full} hex=${bytesToHex(bytes)} ascii="${bytesToAsciiPreview(bytes)}"`);
    }
  });
}
