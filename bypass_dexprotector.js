...
/*
 * Frida 17.x stable bypass for com.dexprotector.detector.envchecks.
 * Run:
 *   ./frida17/bin/frida -U -f com.dexprotector.detector.envchecks -l bypass_dexprotector.js
 */
...
function installOuterHooks(path) {
  ...
  attachOnce(m.base.add(ptr('0x480')), 'outer JNI_OnLoad normal epilogue x0', {
    onEnter() {
      ...
      if (ok) {
        try {
          const installer = globalThis.__dpInstallProviderHooksAfterJni;
          if (typeof installer === 'function') installer(`outer JNI_OnLoad ret-site x0=0x${v.toString(16)}`);
        } catch (e) {
          warn(`[outer JNI_OnLoad RETSITE normal] provider hook installer failed: ${e}`);
        }
      }
    }
  });
...
function main() {
  ...
  log('[mode] JNI_OnLoad/native-only test: Java hooks disabled');
